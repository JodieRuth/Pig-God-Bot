...
def select_system_prompt(user_id: int, scope_key: str) -> str:
    ...
    image_tool_instruction = (
        ...
        "使用本地模型时，先把用户自然语言拆成外观、服装、动作、场景、构图和光照等短英文或日文概念，并调用 drawing_search_tags 查询规范 Danbooru tag；只选择符合用户原意且互不矛盾的结果，related tags 只能作为候选，不能因为高频而擅自加入人体、色情或无关标签。"
        "完成 tag 校正后调用 drawing_search_loras，再调用 drawing_prompt_suggestions；已安装目录没有所需 LoRA 时才调用 drawing_search_civitai，"
...
async def write_pending_restart_message(event: dict[str, Any], message: str, report_services: bool = False) -> None:
    pending = {
        "kind": "restart",
        "message": message,
