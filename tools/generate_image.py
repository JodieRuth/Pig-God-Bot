...
TOOL_DESCRIPTION = "启动一个后台图像生成或图像编辑任务。仅用于合规的图片生成、参考图改图、替换主体、改变风格、图文生图、图像合成等请求。调用本工具时，必须由 LLM 在 prompt 参数中写入最终、完整、可直接发送给图像模型的中文提示词；程序不会再把聊天上下文、系统提示或用户原话追加进绘图接口。需要参考图片时，必须用 image_indexes 明确指定要传入的图片编号；编号只对应当前输入图片顺序，图1 是第一张输入图片，图2 是第二张，不能把历史缓存图、候选拼图编号或别的工具产物混进来。若同时存在多张参考图，而用户没有明确指定某张图，优先使用与当前请求最相关、时间最接近、由触发者本人发送或被明确点名的图片；不要把无关旧图强行混入画面，也不要自行交换图1/图2 顺序。若请求或上下文涉及政治敏感、中国大陆政治不正确、违法违规、暴力恐怖、色情低俗、赌博诈骗、侵犯隐私、规避平台审核、攻击骚扰、仇恨歧视、自伤自杀诱导、未成年人不当内容、伪造证件票据、冒充真实个人或任何 QQ 平台和中国大陆法规不允许的内容，禁止调用此工具，必须直接拒绝。"
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "image_indexes": {
                        "type": "array",
                        "items": {"type": "integer"},
                        "description": "实际传给生图接口的图片编号列表，例如 [1,2]。编号来自当前输入图片顺序：图1 是第一张输入图片，图2 是第二张。只有这里列出的图片会被传入绘图 API；纯文生图必须省略或传空数组。",
                    },
                    "notice": {
                        "type": "string",
