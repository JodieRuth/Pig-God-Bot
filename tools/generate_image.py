from __future__ import annotations

import uuid
from typing import Any


TOOL_DESCRIPTION = "使用通用远程图片生成与编辑模型启动后台任务。任何带参考图的请求都优先使用本工具，包括二次图片修改、局部或整体重绘、修图、扩图、替换主体、保持原图内容改风格和多图合成；本工具也适合写实、产品、场景、文字排版及其他一般向纯文生图。仅当用户没有提供或指定参考图，并明确要求二次元、动漫或游戏角色纯文生图时，优先使用本地模型 drawing_generate_image。对同一请求不得同时调用 generate_image 和 drawing_generate_image。调用本工具时，必须由 LLM 在 prompt 参数中写入最终、完整、可直接发送给图像模型的中文提示词；程序不会再把聊天上下文、系统提示或用户原话追加进绘图接口。需要参考图片时，必须用 image_indexes 明确指定要传入的图片编号；编号只对应当前输入图片顺序，图1 是第一张输入图片，图2 是第二张，不能把历史缓存图、候选拼图编号或别的工具产物混进来。若同时存在多张参考图，而用户没有明确指定某张图，优先使用与当前请求最相关、时间最接近、由触发者本人发送或被明确点名的图片；不要把无关旧图强行混入画面，也不要自行交换图1/图2 顺序。若请求或上下文涉及政治敏感、中国大陆政治不正确、违法违规、暴力恐怖、色情低俗、赌博诈骗、侵犯隐私、规避平台审核、攻击骚扰、仇恨歧视、自伤自杀诱导、未成年人不当内容、伪造证件票据、冒充真实个人或任何 QQ 平台和中国大陆法规不允许的内容，禁止调用此工具，必须直接拒绝。"


def definition(ctx: dict[str, Any]) -> dict[str, Any]:
    return {
        "type": "function",
        "function": {
            "name": "generate_image",
            "description": TOOL_DESCRIPTION,
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
                        "type": "string",
                        "description": "最终、完整、可直接发送给通用远程图片模型的中文提示词。必须由 LLM 根据用户当前请求、必要聊天上下文和所选图片编号自行整合完成；程序不会再额外拼接上下文。若需要编辑或引用图片，必须明确说明图1、图2等编号各自的用途、编辑目标、必须保留的内容和输出要求；一般向纯文生图则直接写完整画面要求。不得包含政治敏感、中国大陆政治不正确、违法违规、色情低俗、隐私侵犯、攻击骚扰等不允许内容。",
                    },
                    "image_indexes": {
                        "type": "array",
                        "items": {"type": "integer"},
                        "description": "实际传给通用远程图片模型的参考图编号列表，例如 [1,2]。编号来自当前输入图片顺序：图1 是第一张输入图片，图2 是第二张。任何二次修改、修图、改风格、替换主体或合成请求都必须列出实际使用的参考图；只有这里列出的图片会被传入绘图 API。纯文生图必须省略或传空数组。",
                    },
                    "notice": {
                        "type": "string",
                        "description": "使用本工具时本轮对话需要发给 QQ 用户的完整回复消息。工具启动成功后程序会立即发送该消息并附加任务 ID，使用自然语言回复即可，就像平时回复那样，但需要告知用户任务已开始需要等待。",
                    },
                },
                "required": ["prompt"],
            },
        },
    }


def info(ctx: dict[str, Any]) -> dict[str, str]:
    item = definition(ctx).get("function", {})
    return {
        "name": str(item.get("name") or "generate_image"),
        "description": str(item.get("description") or ""),
    }


def select_images(images: list[dict[str, Any]], image_indexes: list[Any]) -> list[dict[str, Any]]:
    if not image_indexes:
        return []
    selected: list[dict[str, Any]] = []
    for value in image_indexes:
        try:
            index = int(value)
        except (TypeError, ValueError):
            continue
        if 1 <= index <= len(images):
            record = images[index - 1]
            if record not in selected:
                selected.append(record)
    return selected


async def execute(args: dict[str, Any], runtime: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    prompt = str(args.get("prompt") or "").strip()
    if not prompt:
        return {"ok": False, "content": "生图任务启动失败：缺少工具参数 prompt。"}
    images = select_images(runtime.get("images", []), args.get("image_indexes") or [])
    job_id = uuid.uuid4().hex[:8]
    notice = str(args.get("notice") or "收到，图像任务已开始。").strip() or "收到，图像任务已开始。"
    try:
        queue_result = await ctx["enqueue_image_job"](runtime["event"], job_id, prompt, runtime.get("context_texts", []), images)
        if not queue_result.get("ok"):
            return {"ok": False, "content": str(queue_result.get("content") or "生图任务启动失败。")}
        if queue_result.get("queued"):
            position = int(queue_result.get("position") or 0)
            await ctx["reply"](runtime["event"], f"{notice}\n任务 ID：{job_id}\n当前同时最多生成 2 张，你的任务已进入队列，当前排第 {position} 位。")
        else:
            await ctx["reply"](runtime["event"], f"{notice}\n任务 ID：{job_id}\n当前正在生成。")
    except Exception as exc:
        return {"ok": False, "content": f"生图任务启动失败：{ctx['exception_detail'](exc)}"}
    return {
        "ok": True,
        "answered": True,
        "job_id": job_id,
        "notice": notice,
    }
