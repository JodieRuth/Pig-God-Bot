...
def definition(ctx: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "function": {
            ...
            "description": "识别输入图片里的动漫、游戏或插画人物来源与角色。可以使用 image_indexes 指定图1、图2等图片编号。强制规则：只要当前有可用图片，且用户问题涉及图中人物是谁、角色是谁、出自哪里、人物身份、角色来源、像谁、叫什么，必须先调用本工具获取识别结果，不得仅根据上下文、文件名、画风、模型视觉能力或聊天历史猜测后直接回答。但本工具返回的受到远程模型自身能力限制并不完全可信，因此不能绝对笃定的采信本工具的返回结果。",
            "parameters": {
                "type": "object",
                "properties": {
                    "image_indexes": {
